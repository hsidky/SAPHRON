...
// Tests ising model initialization of a rectangular cell.
TEST(Ising3DModel, RectangularCell)
{
	int xl = 5, yl = 7, zl = 15;
	Ising3DModel m(xl,yl,zl, 1);
	...
	for(int x = 1; x <= xl; x++)
		for(int y = 1; y <= yl; y++)
			for(int z = 1; z <= zl; z++)
			{
				...
				// Check neighbors
				std::vector<double> n1 =
				{(x == xl) ? 1.0 : (double) x + 1.0, (double) y, (double) z};
				std::vector<double> n2 =
				{(x == 1) ? (double) xl : x - 1.0, (double) y, (double) z};
				std::vector<double> n3 =
				{(double) x, (y == yl) ? 1.0 : (double) y + 1.0, (double) z};
				std::vector<double> n4 =
				{(double) x, (y == 1) ? (double) yl : y - 1.0, (double) z};
				std::vector<double> n5 =
				{(double) x, (double) y, (z == zl) ? 1.0 : (double) z + 1.0};
				std::vector<double> n6 =
				{(double) x, (double) y, (z == 1) ? (double) zl : z - 1.0};
