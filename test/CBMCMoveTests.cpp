...
namespace SAPHRON
{
	class CBMCParticleCounter : public ParticleObserver
	{
		public:
			std::map<std::string, int> movecounts;

			CBMCParticleCounter() : movecounts()
			{}
...
TEST(TranslateMove, Selector)
{
	...
	std::map<int, int> startbead = {{s1.GetSpeciesID(), 0}};

	// Not explicit first.
	CBMCMove m1(startbead, 8, false);
...
TEST(TranslateMove, ExplicitDraw)
{
	...
	std::map<int, int> startbead = {{s1.GetSpeciesID(), 0}, {s2.GetSpeciesID(), 0}};

	// Not explicit first.
	CBMCMove m1(startbead, 8, true);
