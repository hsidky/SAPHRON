...
namespace SAPHRON
{
	...
	// Abstract base class representing external constraint potential on a particle. 
	...
	class Constraint : public Serializable
	{
		public:
			...
			static Constraint* BuildConstraint(const Json::Value& json, ForceFieldManager* ffm);
			...
			// ForceField manager. It also adds all initialized pointers to the ccl array passed 
			// in. Throws exception on failure. Object lifetimes in vector are caller's responsibility.
			static void BuildConstraints(const Json::Value& json, ForceFieldManager* ffm, ConstraintList& ccl);
